    ...
    <FijPageShell
      ...
      actions={
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-medium text-black bg-amber-400 hover:bg-amber-300 rounded-lg px-3 py-1.5 transition-colors"
        >
              ...
              <div className="flex items-center gap-3 p-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    {fij.pilot_name && <span className="text-xs text-gray-500">Pilote: {fij.pilot_name}</span>}
                    <span className="text-xs text-gray-500">{fij.member_count || 0} membres</span>
                  </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <Link
                    ...
                    className="text-xs text-amber-400 hover:text-amber-300 px-2 py-1 rounded-lg hover:bg-amber-400/5 transition-colors"
                  >
                    Ouvrir
                  </Link>
                </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={onClose}>
      <motion.div
        ...
        className="bg-zinc-950 border border-white/10 rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"
      >
        <div className="sticky top-0 bg-zinc-950 border-b border-white/5 px-5 py-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-white">{fij ? 'Modifier la FIJ' : 'Nouvelle FIJ'}</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Jour de réunion</label>
          ...
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Nom du pilote</label>
            <input className={inputCls} value={form.pilot_name} onChange={e => setForm(f => ({ ...f, pilot_name: e.target.value }))} placeholder="Jean Dupont" />
          </div>
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Email du pilote</label>
          ...
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Nombre de membres</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.is_nantes} onChange={e => setForm(f => ({ ...f, is_nantes: e.target.checked }))} className="w-4 h-4 rounded accent-amber-400" />
            <span className="text-sm text-gray-400">FIJ basée à Nantes</span>
          </label>
        </div>
        <div className="sticky bottom-0 bg-zinc-950 border-t border-white/5 px-5 py-4 flex gap-2">
          <button onClick={onClose} className="flex-1 text-sm text-gray-400 border border-white/10 rounded-xl py-2.5 hover:bg-white/5 transition-colors">Annuler</button>
          <button onClick={save} disabled={saving || !form.name || !form.city} className="flex-1 text-sm font-medium text-black bg-amber-400 hover:bg-amber-300 rounded-xl py-2.5 transition-colors disabled:opacity-50">
