              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {alert.status === 'resolved' && <CheckCircle className="w-3.5 h-3.5 text-success flex-shrink-0" />}
                  </div>
                  {alert.description && <p className="text-xs text-muted-foreground mt-0.5">{alert.description}</p>}
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-surface text-muted-foreground">{TYPE_LABELS[alert.type] || alert.type}</span>
                    {alert.fij_name && <span className="text-[10px] text-muted-foreground">· {alert.fij_name}</span>}
                  </div>
                </div>
              </div>
              {isCoord && alert.status !== 'resolved' && (
                <div className="flex gap-2 mt-2">
                  <button onClick={() => resolve(alert)} className="flex items-center gap-1 text-xs text-success hover:text-success/80 border border-success/30 rounded-lg px-2.5 py-1 transition-colors">
                    <CheckCircle className="w-3 h-3" /> Résoudre
                  </button>
                  {alert.severity !== 'critical' && (
                    <button onClick={() => escalate(alert)} className="flex items-center gap-1 text-xs text-danger hover:text-danger/80 border border-danger/30 rounded-lg px-2.5 py-1 transition-colors">
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={onClose}>
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-sm font-semibold text-foreground">Signaler un besoin</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input className={inputCls} value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Résumé du besoins" />
          </div>
          <div>
        ...
        <div className="px-5 py-4 border-t border-border flex gap-2">
          ...
          <button onClick={save} disabled={saving || !form.title} className="flex-1 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-xl py-2.5 disabled:opacity-50">
            {saving ? '...' : 'Signaler'}
          </button>
        </div>
      </motion.div>
