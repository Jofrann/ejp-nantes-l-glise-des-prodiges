        ...
        <div className="space-y-2">
          {visible.map((doc, i) => (
            <motion.a
              ...
              className="flex items-center gap-3 bg-white/3 border border-white/8 rounded-xl p-3 hover:border-white/15 transition-colors group"
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white truncate group-hover:text-amber-400 transition-colors">{doc.title}</p>
                {doc.description && <p className="text-xs text-gray-500 truncate">{doc.description}</p>}
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/5 text-gray-400">{doc.category}</span>
                  {doc.visibility && <span className="text-[10px] text-gray-600">· {doc.visibility}</span>}
                </div>
              </div>
            </motion.a>
          ))}
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={onClose}>
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
        ...
        <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-white">Nouveau document</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Fichier</label>
            {form.file_url ? (
              <div className="flex items-center gap-2 bg-green-500/5 border border-green-500/15 rounded-xl px-3 py-2.5">
                <FileText className="w-4 h-4 text-green-400" />
                <span className="text-xs text-green-400 flex-1 truncate">Fichier téléversé</span>
                <button onClick={() => setForm(f => ({ ...f, file_url: '' }))} className="text-xs text-red-400">Retirer</button>
              </div>
            ) : (
              <label className="flex items-center justify-center gap-2 border border-dashed border-white/15 rounded-xl py-4 cursor-pointer hover:bg-white/5 transition-colors">
                <Upload className="w-4 h-4 text-gray-500" />
                <span className="text-xs text-gray-500">{uploading ? 'Téléversement...' : 'Choisir un fichier'}</span>
        ...
        <div className="px-5 py-4 border-t border-white/5 flex gap-2">
          ...
          <button onClick={save} disabled={saving || !form.title || !form.file_url} className="flex-1 text-sm font-medium text-black bg-amber-400 hover:bg-amber-300 rounded-xl py-2.5 disabled:opacity-50">
            {saving ? '...' : 'Publier'}
          </button>
        </div>
      </motion.div>
